...
audioResourceWidgetDyn a = divClass "resourceWidget" $ do
    file' <- mapDyn resourceFileName a
    dur <-  mapDyn showAudioDuration a
    tags' <-  mapDyn showTags a
    scope' <- mapDyn (T.pack . show . resourceScope) a
...
videoResourceWidgetDyn a = divClass "resourceWidget" $ do
    file' <- mapDyn resourceFileName a
    dur <- mapDyn showVideoDuration a
    shape <- mapDyn showVideoShape a
    tags' <- mapDyn showTags a
    scope' <-  mapDyn (T.pack . show . resourceScope) a
    divClass "resourceName" $ dynText file'
    divClass "resourceDur" $ dynText dur
