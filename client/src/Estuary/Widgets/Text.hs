...
textNotationWidget ctx e rows i delta = divClass "textPatternChain" $ do -- *** TODO: change css class
  ...
  (d,evalButton,infoButton) <- divClass "fullWidthDiv" $ do
    ...
    d' <- dropdown initialParser parserMap $ ((def :: DropdownConfig t TidalParser) & attributes .~ constDyn ("class" =: "code-font background primary-color")) & dropdownConfig_setValue .~ parserFuture
