...
textNotationWidget ctx e rows i delta = divClass "textPatternChain" $ do -- *** TODO: change css class
  ...
  (d,evalButton,infoButton) <- divClass "fullWidthDiv" $ do
    ...
    d' <- dropdown initialParser parserMap $ ((def :: DropdownConfig t TidalParser) & attributes .~ constDyn ("class" =: "code-font background primary-color primary-borders")) & dropdownConfig_setValue .~ parserFuture
  ...
  (edit,eval) <- divClass "labelAndTextPattern" $ do
    ...
    (textValue,textEvent,shiftEnter) <- hideableWidget textVisible "width-100-percent" $ textAreaWidgetForPatternChain rows initialText textFuture
