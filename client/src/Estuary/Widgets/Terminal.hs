...
terminalWidget ctx deltasUp deltasDown hints = divClass "terminal" $ mdo
  ...
  (sendButton,inputWidget) <- divClass "terminalHeader code-font background-color foreground-color" $ do
    ...
    let attrs = constDyn $ fromList [("class","background-color foreground-color code-font"),("style","width: 100%")]
