...
sequencerRow (iVal,vals) edits = elClass "tr" "sequencerRow" $ do
  ...
  let textInputAttrs = singleton "class" "sequencer-textarea"
  deleteMe <- elClass "td" "delete" $ button "-" -- clickableTdClass (constDyn " - ") (constDyn "delete") ()
  rowInput <- elClass "td" "sequencer-textarea" $ textInput $ def & textInputConfig_initialValue .~ (T.pack iVal) & textInputConfig_setValue .~ strUpdate & textInputConfig_attributes .~ (constDyn empty)
...
sequencerButton pos val edits = mdo
  ...
  attrs <- mapDyn (\x-> singleton "class" $ if x then "sequencerButton activated" else "sequencerButton sequencer-pads") isActive
  return isActive
