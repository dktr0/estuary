...
defineNumRowsOrColumns :: Int -> [T.Text]
defineNumRowsOrColumns n = Prelude.take n (repeat "auto")

listToString :: [T.Text] -> T.Text
listToString [] = ""
listToString [x] = x
listToString (x:xs) = x <> " " <> (listToString xs)

setNumColumns:: Int -> T.Text
setNumColumns n =  "grid-template-columns: " <> (listToString $ defineNumRowsOrColumns n) <> ";"

setNumRows:: Int -> T.Text
setNumRows n =  "grid-template-rows: " <> (listToString $ defineNumRowsOrColumns n) <> ";"

setColumnsAndRows :: Int -> Int ->  T.Text
setColumnsAndRows n m = (setNumColumns n) <> (setNumRows m)
