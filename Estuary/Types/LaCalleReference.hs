...
about = divClass "aboutWrapper" $ do
    title <- divClass "aboutTitle" $ text  "Welcome to"
    x <- divClass "AboutButton" $ button "LaCalle"
    welcomeVisible <- toggle False x
    hideableWidget welcomeVisible "aboutText" $ text "A mini live coding esolang developed in Lima (Peru) by Ivanka Cotrina, using slang characteristic of that city’s working-class neighbourhoods."
    return ()
