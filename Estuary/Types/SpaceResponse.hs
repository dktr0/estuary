module Estuary.Types.SpaceResponse where
...
data SpaceResponse v =
  ...
  SpaceClientCount Int  

instance JSON v => JSON (SpaceResponse v) where
  ...
  showJSON (SpaceClientCount x) = encJSDict [("SpaceClientCount",x)]
  ...
  readJSON (JSObject x) | firstKey x == "SpaceClientCount" = SpaceClientCount <$> valFromObj "SpaceClientCount" x  
  readJSON (JSObject x) | otherwise = Error $ "Unable to parse JSObject as SpaceResponse: " ++ (show x)
  readJSON _ = Error "Unable to parse non-JSObject as SpaceResponse"

justEditsInZone :: Int -> [SpaceResponse a] -> [a]
justEditsInZone z1 = mapMaybe f
  where
    f (ZoneResponse (Sited z2 (Edit a))) | z1 ==z2 = Just a
    f _ = Nothing

justChats :: [SpaceResponse v] -> [(String,String)]
justChats = mapMaybe f
  where f (Chat x y) = Just (x,y)
        f _ = Nothing

justViews :: [SpaceResponse v] -> [Sited String View]
