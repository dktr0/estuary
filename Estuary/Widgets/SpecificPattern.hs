...
specificContainer (Bandq x) e = G.generalContainerLive  (G.popupDoubleWidget 0 22000 10) x never >>= mapDyn (\(x,ev)->(Bandq x,(() <$) ev))
specificContainer (Begin x) e = G.generalContainerLive  (G.popupDoubleWidget 0 1 0.05) x never >>= mapDyn (\(x,ev)->(Begin x,(() <$) ev))
specificContainer (Delay x) e = G.generalContainerLive    (G.popupDoubleWidget 0 1 0.05) x never >>= mapDyn (\(x,ev)->(Delay x,(() <$) ev))
specificContainer (Delayfeedback x) e = G.generalContainerLive (G.popupDoubleWidget 0 1 0.05) x never >>= mapDyn (\(x,ev)->(Delayfeedback x,(() <$) ev))
specificContainer (Delaytime x) e = G.generalContainerLive (G.popupDoubleWidget 0 1 0.05) x never >>= mapDyn (\(x,ev)->(Delaytime x,(() <$) ev))
specificContainer (End x) e = G.generalContainerLive (G.popupDoubleWidget 0 1 0.05) x never >>= mapDyn (\(x,ev)->(End x,(() <$) ev))
specificContainer (Gain x) e = G.generalContainerLive (G.popupDoubleWidget 0 1 0.05) x never >>= mapDyn (\(x,ev)->(Gain x,(() <$) ev))
specificContainer (Hresonance x) e = G.generalContainerLive (G.popupDoubleWidget 0 1 0.05) x never >>= mapDyn (\(x,ev)->(Hresonance x,(() <$) ev))
specificContainer (Pan x) e = G.generalContainerLive (G.popupDoubleWidget 0 1 0.05) x never >>= mapDyn (\(x,ev)->(Pan x,(() <$) ev))
specificContainer (Resonance x) e = G.generalContainerLive (G.popupDoubleWidget 0 1 0.05) x never >>= mapDyn (\(x,ev)->(Resonance x,(() <$) ev))
specificContainer (Shape x) e = G.generalContainerLive (G.popupDoubleWidget 0 1 0.05) x never >>= mapDyn (\(x,ev)->(Shape x,(() <$) ev))
specificContainer (Speed x) e = G.generalContainerLive (G.popupDoubleWidget (-999) 999 0.5) x never >>= mapDyn (\(x,ev)->(Speed x,(() <$) ev))
specificContainer (Up x) e = G.generalContainerLive (G.popupDoubleWidget (-132) 132 1) x never >>= mapDyn (\(x,ev)->(Up x,(() <$) ev))
specificContainer (Bandf x) e = G.generalContainerLive (G.popupIntWidget 0 25000 10) x never >>= mapDyn (\(x,ev)->(Bandf x,(() <$) ev))
specificContainer (Coarse x) e = G.generalContainerLive (G.popupIntWidget 0 24 1) x never >>= mapDyn (\(x,ev)->(Coarse x,(() <$) ev))
specificContainer (Crush x) e = G.generalContainerLive (G.popupIntWidget 0 16 1) x never >>= mapDyn (\(x,ev)->(Crush x,(() <$) ev))
specificContainer (Estuary.Tidal.Types.Cut x) e = G.generalContainerLive (G.popupIntWidget (-50) 50 1) x never >>= mapDyn (\(x,ev)->(Estuary.Tidal.Types.Cut x,(() <$) ev))
specificContainer (Cutoff x) e = G.generalContainerLive (G.popupIntWidget 0 25000 10) x never >>= mapDyn (\(x,ev)->(Cutoff x,(() <$) ev))
specificContainer (Hcutoff x) e = G.generalContainerLive (G.popupIntWidget 0 25000 10) x never >>= mapDyn (\(x,ev)->(Hcutoff x,(() <$) ev))
specificContainer (Loop x) e = G.generalContainerLive (G.popupIntWidget 0 1024 1) x never >>= mapDyn (\(x,ev)->(Loop x,(() <$) ev))
