...
specificContainer (Accelerate x) e = G.generalContainerLive (G.popupDoubleWidget (-500) 500 1) x never >>= mapDyn (\(x,ev,h)->(Accelerate x,(() <$) ev,h))
specificContainer (Bandq x) e = G.generalContainerLive  (G.popupDoubleWidget 0 22000 10) x never >>= mapDyn (\(x,ev,h)->(Bandq x,(() <$) ev,h))
specificContainer (Begin x) e = G.generalContainerLive  (G.popupDoubleWidget 0 1 0.05) x never >>= mapDyn (\(x,ev,h)->(Begin x,(() <$) ev,h))
specificContainer (Delay x) e = G.generalContainerLive    (G.popupDoubleWidget 0 1 0.05) x never >>= mapDyn (\(x,ev,h)->(Delay x,(() <$) ev,h))
specificContainer (Delayfeedback x) e = G.generalContainerLive (G.popupDoubleWidget 0 1 0.05) x never >>= mapDyn (\(x,ev,h)->(Delayfeedback x,(() <$) ev,h))
specificContainer (Delaytime x) e = G.generalContainerLive (G.popupDoubleWidget 0 1 0.05) x never >>= mapDyn (\(x,ev,h)->(Delaytime x,(() <$) ev,h))
specificContainer (End x) e = G.generalContainerLive (G.popupDoubleWidget 0 1 0.05) x never >>= mapDyn (\(x,ev,h)->(End x,(() <$) ev,h))
specificContainer (Gain x) e = G.generalContainerLive (G.popupDoubleWidget 0 1 0.05) x never >>= mapDyn (\(x,ev,h)->(Gain x,(() <$) ev,h))
specificContainer (Hresonance x) e = G.generalContainerLive (G.popupDoubleWidget 0 1 0.05) x never >>= mapDyn (\(x,ev,h)->(Hresonance x,(() <$) ev,h))
specificContainer (Pan x) e = G.generalContainerLive (G.popupDoubleWidget 0 1 0.05) x never >>= mapDyn (\(x,ev,h)->(Pan x,(() <$) ev,h))
specificContainer (Resonance x) e = G.generalContainerLive (G.popupDoubleWidget 0 1 0.05) x never >>= mapDyn (\(x,ev,h)->(Resonance x,(() <$) ev,h))
specificContainer (Shape x) e = G.generalContainerLive (G.popupDoubleWidget 0 1 0.05) x never >>= mapDyn (\(x,ev,h)->(Shape x,(() <$) ev,h))
specificContainer (Speed x) e = G.generalContainerLive (G.popupDoubleWidget (-999) 999 0.5) x never >>= mapDyn (\(x,ev,h)->(Speed x,(() <$) ev,h))
specificContainer (Up x) e = G.generalContainerLive (G.popupDoubleWidget (-132) 132 1) x never >>= mapDyn (\(x,ev,h)->(Up x,(() <$) ev,h))
specificContainer (Bandf x) e = G.generalContainerLive (G.popupIntWidget 0 25000 10) x never >>= mapDyn (\(x,ev,h)->(Bandf x,(() <$) ev,h))
specificContainer (Coarse x) e = G.generalContainerLive (G.popupIntWidget 0 24 1) x never >>= mapDyn (\(x,ev,h)->(Coarse x,(() <$) ev,h))
specificContainer (Crush x) e = G.generalContainerLive (G.popupIntWidget 0 16 1) x never >>= mapDyn (\(x,ev,h)->(Crush x,(() <$) ev,h))
specificContainer (Estuary.Tidal.Types.Cut x) e = G.generalContainerLive (G.popupIntWidget (-50) 50 1) x never >>= mapDyn (\(x,ev,h)->(Estuary.Tidal.Types.Cut x,(() <$) ev,h))
specificContainer (Cutoff x) e = G.generalContainerLive (G.popupIntWidget 0 25000 10) x never >>= mapDyn (\(x,ev,h)->(Cutoff x,(() <$) ev,h))
specificContainer (Hcutoff x) e = G.generalContainerLive (G.popupIntWidget 0 25000 10) x never >>= mapDyn (\(x,ev,h)->(Hcutoff x,(() <$) ev,h))
specificContainer (Loop x) e = G.generalContainerLive (G.popupIntWidget 0 1024 1) x never >>= mapDyn (\(x,ev,h)->(Loop x,(() <$) ev,h))
specificContainer (N x) e = G.generalContainerLive (G.popupIntWidget 0 50 1) x never >>= mapDyn (\(x,ev,h)->(N x,(() <$) ev,h))
