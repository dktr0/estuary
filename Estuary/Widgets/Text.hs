...
textGeneralContainer :: MonadWidget t m => GeneralPattern a -> Event t () -> m (Dynamic t (GeneralPattern a, Event t ()))
...
textPatternChain _ _ = do
  ...
  sn <- combineDyn (,) s n
  combineDyn (\(a,b) c -> PatternChain' a Merge $ PatternChain' b Merge $ PatternChain c) sn up
  where
